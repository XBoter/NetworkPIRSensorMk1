...
void Configuration::Run()
{
    ...
    else
    {
        ...
        if (resetOrNotConfigured)
        {
            ...
            case 2:
                WiFi.softAP("Network PIR Sensor Mk1");
                state++;
                break;
...
void inputForm()
{
    ...
    String WEP_PAGE = "<!DOCTYPE html><!-- After webpage changes go to https://www.textfixer.com/html/compress-html-compression.php and compress the html page before copying code to arduino--><html><head> <meta name='viewport' content='width=device-width, initial-scale=1.0, maximum-scale=2.0, user-scalable=no'> <style> body { background-color: #616161; width: 100vw; height: 100vh; display: flex; justify-content: center; align-items: center; } form { display: flex; justify-content: center; align-items: center; flex-direction: column; } h1, label, input { color: #ffffff; font-family: Arial, Helvetica, sans-serif; font-size: 20px; } h1 { text-align: center; font-size: 35px; padding: 10px; color: #aeea00; } input { color: #000000; } #submitButton { border-width: 0; outline: none; border-radius: 2px; box-shadow: 0 1px 4px rgba(0, 0, 0, .6); margin-top: 10px; padding: 10px; width: 50%; background-color: #2ecc71; color: #ecf0f1; } td { padding: 5px; } </style></head><body> <div> <h1>Network PIR Sensor Configuration</h1> <form action='/final' method='post'> <table> <tr> <td><label>WiFi SSID</label></td> <td><input type='text' required name='wifiSSID'></td> </tr> <tr> <td><label>WiFi Password</label></td> <td><input type='text' required name='wifiPassword'></td> </tr> <tr> <td><label>MQTT Broker IP Address</label></td> <td><input type='text' required pattern='^([0-9]{1,3}\\.){3}[0-9]{1,3}$' name='mqttBrokerIpAddress'> </td> </tr> <tr> <td><label>MQTT Broker Port</label></td> <td><input type='number' required min='1' name='mqttBrokerPort'></td> </tr> <tr> <td><label>MQTT Broker Username</label></td> <td><input type='text' required name='mqttBrokerUsername'></td> </tr> <tr> <td><label>MQTT Broker Password</label></td> <td><input type='text' required name='mqttBrokerPassword'></td> </tr> <tr> <td><label>MQTT Client Name</label></td> <td><input type='text' required name='mqttClientName'></td> </tr> <tr> <td><label>MQTT LED Controller Client Name</label></td> <td><input type='text' required name='mqttLEDControllerClientName'></td> </tr> </table> <input id='submitButton' type='submit' value='Submit'> </form> </div></body></html>";
...
void inputFormFilled()
{
    ...
    if (server.hasArg("mqttLEDControllerClientName"))
    {
        data.mqttLEDControllerClientName = server.arg("mqttLEDControllerClientName");
    }
    data.isConfigured = false;
...
void Configuration::saveConfig()
{
    ...
    // ==== mqttLEDControllerClientName ==== //
    if (!file.println(data.mqttLEDControllerClientName))
    {
        Serial.println(F("mqttLEDControllerClientName failed to save"));
    }
        ...
        if (x == 13)
        {
            ...
            if (x == 10)
            {
                ...
                case 7:
                    ...
                    // ==== mqttLEDControllerClientName ==== //
                case 8:
                    data.mqttLEDControllerClientName = message;
                    break;
                }
                message = "";
            }
